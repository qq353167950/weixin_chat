...
- **更新检查**：启动时仅加载当前版本号；点击顶栏版本号后手动检查，优先程序内更新，失败时显示可复制的 GitHub 安装包地址
...
- **输入历史**：搜索关键词自动记录最近 3 条，聚焦输入框快速选择
- **实例限制**：最多允许 2 个窗口同时运行，避免资源浪费和误启动

## 🚀 快速开始

### 方式一：下载安装（推荐，免装 Python）

到 [Releases](../../releases) 按平台下载：

| 文件 | 平台 | 说明 |
|------|------|------|
| `wechat-assistant-setup-v*.exe` | Windows 10/11 | **推荐**：安装向导（选目录、桌面快捷方式、卸载器） |
| `wechat-assistant.exe` | Windows 10/11 | 绿色版：放任意文件夹双击即用 |
| `wechat-assistant-macos.zip` | macOS | 解压得 `公众号助手.app`，拖入「应用程序」 |

首次启动有**配置引导**：跟着动画填写作模型 Key 与公众号 AppID/AppSecret 即可开写。

数据位置：v1.7.3 起安装版数据（.env / runs）在**安装目录**；旧版装在 Program Files 的在 `%APPDATA%\公众号助手`；绿色版在 exe 同目录；macOS 在 `~/Library/Application Support/公众号助手`。

> Windows 需系统自带 Edge WebView2（Win10/11 绝大多数已内置，缺失自动回退浏览器模式）。exe 未做代码签名，SmartScreen 提示时选「仍要运行」；macOS 首次打开右键 .app → 打开。
...
# 本地打包（Windows：exe + 安装包；mac 上运行 pyinstaller build.spec 出 .app）
build_exe.bat
